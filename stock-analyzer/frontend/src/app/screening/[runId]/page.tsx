...
export default function ScreeningResultsPage() {
  ...
  async function updateStage(resultId: number, stage: string) {
    if (runId === null) return
    try {
      const updated = await apiFetch<StockResult>(
        `/screening/${runId}`,
        {
          method: "PATCH",
  ...
  async function bulkReject() {
    ...
    setResults((prev) =>
      prev.map((r) =>
        ids.includes(r.id) ? { ...r, stage: "rejected" } : r
      )
    )
    setSelected(new Set())
    // Persist
            ...
            <div key={stock.id} className="relative">
              ...
              {stock.stage === "rejected" && showRejected && (
                <button
                  ...
                  className="absolute right-3 top-3 rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-600 shadow-sm hover:bg-gray-50"
                >
                  Un-reject
                </button>
              )}
